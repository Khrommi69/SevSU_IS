...
int main() {
    ...
    double Fn,      //F(n)
           ...
           n;      //Значение аргумента функций трудоемкости (n)
    ...
    n = Nbegin; //Аргумент функций равен левой границе интервала

    fprintf(stream, "n,      F(n),      G(n),      ATg_FG,      ATg_GF,      pi,      Delta,      Theta,      O_large \n\n");

    while (n <= Nend) {
        Fn = (powl(2, (n-4))) / sqrtl(n);   //Расчет значения функции F(n)
        Gn = powl(1.618, n);                   //Расчет значения функции G(n)
        ATg_FG = atan(Fn/Gn);
        ATg_GF = atan(Gn/Fn);
        pi = ATg_FG - ATg_GF;
        ...
        fprintf(stream, "%lf %lf \t %lf\t %lf %lf %lf %lf %lf %lf\n",
                n, Fn, Gn, ATg_FG, ATg_GF, pi, Delta, Theta, O_large);
        n = n + step; //Получение следующего значения аргумента
    }
